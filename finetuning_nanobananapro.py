...
# 1. Charge le modèle pré-entraîné (AVEC classifier entraîné)
trained_model_path = "./best_model.pth"
trained_model = load_mobilenetv3_model(trained_model_path, num_classes=2)

# 2. Gèle toutes les couches sauf le classifier (par défaut tout est dégelé)
# Gèle les features
for param in trained_model.features.parameters():
    param.requires_grad = False

# Le classifier reste dégelé (requires_grad=True par défaut)

trained_model = trained_model.to(DEVICE)

print(f"Modèle chargé avec classifier entraîné")

# 4. Prépare le nouveau dataset
...
# 5. Définit la fonction de perte
loss_fcn = nn.CrossEntropyLoss()

# 6. Configure l'optimiseur (classifier uniquement, learning rate bas pour fine-tuning)
optimizer = optim.Adam(
    trained_model.classifier.parameters(),
    lr=0.0001  # Plus bas car on fine-tune un classifier déjà entraîné
)

# 7. Entraîne le modèle
continued_model = helper_utils.training_loop_with_best_model(
    trained_model,
    new_train_loader,
    new_val_loader,
    loss_fcn,
